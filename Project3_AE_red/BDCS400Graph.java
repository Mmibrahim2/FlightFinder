...
public class BDCS400Graph<NodeType,EdgeType extends Number> implements GraphADT<NodeType,EdgeType> {
    ...
    public BDCS400Graph() { vertices = new Hashtable<>(); }
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
		   ...
		   NodeType s = copyPath.dataSequence.get(i);
                dataSequence.add(s);
            }
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        ...
        if (!vertices.containsKey(start)) {
            throw new NoSuchElementException("ERROR: Start vertices not found");
        }

        // check end
        if (!vertices.containsKey(end)) {
            throw new NoSuchElementException("ERROR: End vertices not found");
        }

        // create vertex linked list of visited
        List<Vertex> vs = new LinkedList<>();
        Vertex starts = vertices.get(start);
        // create paths priority queue
        PriorityQueue<Path> paths = new PriorityQueue<Path>();

        // Starting path
        Path path = new Path(starts);
        // add path
        paths.add(path);
        ...
        while(!paths.isEmpty()) {
            ...
            vs.add(remove.end);
            if (remove.end.data.equals(end)) {
                return remove;
            }
            else {
                // end
                List<Edge> edges = remove.end.edgesLeaving;
                for (int i = 0; i < edges.size(); i++) {
                    // is it visited
                    Vertex f = edges.get(i).target;
                    if (!vs.contains(f)) {
                        Path p = new Path(remove, edges.get(i));
                        paths.add(p);
                    }
                }
            }
        ...
        throw new NoSuchElementException("ERROR: Path not found");
    }
